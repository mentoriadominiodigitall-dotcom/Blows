...
import { whatsappConfigured, sendWhatsAppMessage } from "@/lib/integrations/whatsapp";
import {
  CATEGORY_META,
  priorityForAmount,
  type LostCategory,
  type LostCategoryKey,
  type LostItem,
  type LostMoneySnapshot,
} from "@/lib/lost-money/types";
import { daysSince, num } from "@/lib/utils";
import { assertPermission, requireTenant, writeAudit } from "./tenant";

function itemPriority(amount: number, last: string | null): LostItem["priority"] {
  return priorityForAmount(amount, daysSince(last) ?? 0);
}

export const getLostMoney = createServerFn({ method: "GET" })
  .middleware([authMiddleware])
  .handler(async ({ context }): Promise<LostMoneySnapshot> => {
    try {
      const { sql, tenant } = await requireTenant(context.userId);
      const [settings] = await sql<{
        inactive_days: number;
        lead_idle_days: number;
        quote_idle_days: number;
        repurchase_multiplier: string;
      }>`
...
export const createRecoveryCampaign = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator((input: {
    name: string;
  ...
  .handler(async ({ context, data }) => {
    try {
      ...
      if (campaign.channel !== "whatsapp") {
        throw new AppError("VALIDATION", "Canal ainda não suportado.");
      }
      ...
      const nextStatus = whatsappConfigured() ? campaign.status : "needs_config";
      await sql`
        update campaigns set status = ${nextStatus}
